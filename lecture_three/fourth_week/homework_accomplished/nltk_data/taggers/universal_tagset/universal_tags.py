...
def fileids(lang=''):
    ...
    ''' 
    return [re.match(r'.*[/]([^/\\]+)[.]map', p).group(1) 
    for p in glob.glob(MAP_DIR + '/{}-*.map'.format(lang.lower()))]
...
def mapping(fileid):
    ...
    >>> mapping('ru-rnc')=={'!': '.', 'A': 'ADJ', 'AD': 'ADV', 'C': 'CONJ',
    'COMP': 'CONJ', 'IJ': 'X', 'NC': 'NUM', 'NN': 'NOUN', 'P': 'PRON', 'PTCL': 'PRT',
    'V': 'VERB', 'VG': 'VERB', 'VI': 'VERB', 'VP': 'VERB', 'YES_NO_SENT': 'X', 'Z': 'X'}
    True
    '''
